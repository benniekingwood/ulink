...
class ThumbsController extends AppController {
    ...
    function index() {
        ...
        if (empty($this->request->params['url']['src'])) {
            $this->log('thumbs_controller->index() - file: ' . $this->request->params['url']['src'] . ', does not exist', 'debug');
            die('File does not exist');
        }
        unset($this->request->params['url']['url']);

        $this->request->params['url']['src'] = ROOT . $this->request->params['url']['src'];

        if (is_readable($this->request->params['url']['src'])) {
            uses('folder');
