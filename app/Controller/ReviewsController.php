...
//ef 	
    ...
    function admin_review_index() {
        ...
        if (isset($this->request->data['Review']['searchText'])) {
            $this->Session->write('advancedReviewSearch', $this->request->data['Review']['searchText']);
            $searchText = $this->request->data['Review']['searchText'];
        } elseif ($this->Session->check('advancedReviewSearch')) {

            if (isset($this->request->params['named']['page'])) {
                $searchText = $this->request->data['AdvancedSearch'] = $this->Session->read('advancedReviewSearch');
            } else {
...
//ef
    ...
    function admin_review_edit($id = null) {
        ...
        if (empty($this->request->data)) {
            $edirRev = $this->request->data = $this->Review->read('', $id);
...
//ef alltextreview
    ...
    function writereview() {
        if (!empty($this->request->data)) {
            ...
            $this->request->data['Review']['rating'] = $this->request->data['Review']['ratingnew'];
            $revtype = $this->request->data['Review']['type'];
            $revSid = $this->request->data['Review']['school_id'];
            $revUid = $this->request->data['Review']['user_id'];
            ...
            if ($school_id[0]['User']['school_id'] != $revSid) {
                ...
                if (!empty($text_review)) {
                    //	$this->request->data['Review']['id']		=	$text_review['Review']['id'];
                    $edit = 1;
                }
...
// ef   
    ...
    function uploadreview_step($item_id = null) {
        ...
        if (!empty($this->request->data)) {
            ...
            $this->request->data['Review']['description'] = str_replace('<p>', '', $this->request->data['Review']['description']);
            $this->request->data['Review']['description'] = str_replace('</p>', '', $this->request->data['Review']['description']);
            //	echo $this->request->data['Review']['description'];


            $this->Session->write("title_data", $this->request->data['Review']['title']);
            $this->Session->write("description", $this->request->data['Review']['description']);
...
//ef 
    ...
    function editvideo_step($item_id = null) {
        ...
        if (!empty($this->request->data)) {
            ...
            $token = $this->request->data['Review']['link'];
            ...
            $item = $this->request->data['Review']['school_id'];
            $videoEntry->setVideoDescription($this->request->data['Review']['description']);
            $videoEntry->setVideoTitle($this->request->data['Review']['title']);
    ...
    function uploadvideoreview($item_id=null) {
        ...
        if ($_GET['status'] != "200" && $_GET['status'] != "400") {
            ...
            $sessVar = $this->Auth->user();
            $this->request->data['Review']['user_id'] = $sessVar['User']['id'];
            $this->request->data['Review']['school_id'] = $item;
            $this->request->data['Review']['link'] = $_GET['id'];
            $this->request->data['Review']['rating'] = $rating;
            $this->request->data['Review']['title'] = $title_data;
            $this->request->data['Review']['description'] = $description;
            $this->request->data['Review']['id'] = $_COOKIE['reviewid'];
            $revtype = $this->request->data['Review']['type'];
            $revSid = $this->request->data['Review']['school_id'];
            $revUid = $this->request->data['Review']['user_id'];
            ...
            if (count($reviewnum)) {
                ...
                $this->request->data['Review']['id'] = $reviewnum['Review']['id'];
                ...
                unset($this->request->data['Review']['id']);
                if ($this->Review->save($this->request->data)) {
                    $this->Session->setFlash('Your video review has been uploaded and has been sent to the administrator for its approval');
                    $this->redirect(array('action' => 'allvideoreview/' . $item));
                } else {
...
// ef of function to upload video on youtube
    ...
    function checkValidReview($id=null, $returntype, $conditions=null) {
        ...
        //$this->autoRender=false;
        if (isset($this->request->data['Review']['id'])) {           // this is set id if manipulation is done in html page
            $id = $this->request->data['Review']['id'];
        }
