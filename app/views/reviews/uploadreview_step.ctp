...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#VideoReviewWritereviewForm").validate({
...
<div class="profile_image">
    ...
    <?php if ($Shoolreview[0]['School']['image_url'] != '' && file_exists(WWW_ROOT . '/img/files/schools/' . $Shoolreview[0]['School']['image_url'])) {
        ?>
        <div class="userProfileimage" id="checkImage"><?php echo $html->image('files/schools/' . $Shoolreview[0]['School']['image_url'] . '', array('alt' => '', 'height' => '100', 'width' => '100')); ?><br/>

        <?php } else { ?>
            <div class="userProfileimage">
                <img alt="" border="0" src="<?php e($html->url('/thumbs/index/')); ?>?src=/app/webroot/img/files/users/noImage.jpg&w=225&h=225" /></div>
        <?php } ?>
                ...
                <div class="videocontent">
                    ...
                    <div class="videocontent"><label>Rate School</label>
                        ...
                        <?php for ($j = 1; $j < 6; $j++) { ?>	
                            ...
                            <?php echo $html->image('img_rateMiniStarOff.gif', array('onclick' => 'setRatingLevelVideo(' . $j . ')', 'onMouseOut' => 'resetRatingLevelVideo()', 'onmouseover' => 'setDisplayRatingLevelVideo(' . $j . ')', 'name' => 'starkk' . $j . '')); ?>
                        ...
                        <?php if ($reviews[0]['Review']['rating'] != 0) { ?>
                            <?php e($form->input('Review.rating', array('type' => 'hidden', 'value' => $reviews[0]['Review']['rating']))); ?>
                        <?php } else { ?>
                            <?php //echo "shakasasti"; ?>
                            <?php e($form->input('Review.rating', array('type' => 'hidden'))); ?>
                        <?php } ?> 

                        <?php echo $form->input('Review.ratingInfonew', array('type' => 'hidden', 'label' => false, 'div' => false, 'hiddenField' => false)) ?>
                    </div>
                    <?php echo $form->input('Review.title', array('onkeyup' => 'setTitleCookie(this.value)', 'value' => $reviews[0]['Review']['title'], 'label' => 'Title')); ?>
                    <?php //echo $form->input('Review.description',array('onkeyup'=>'setDescriptionCookie(this.value)','value'=>$reviews[0]['Review']['description'],'label'=>'Comment','type'=>'textarea'));?>			      
                    ...
                    <?php e($form->input('Review.autoval', array('id' => 'autoval', 'value' => $RandCaptcha, 'class' => 'captchaImagevideo', 'disabled' => 'disabled', 'label' => '&nbsp;'))) ?>
