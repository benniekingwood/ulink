...
<div class="content">
  ...
  <?php 
        if ($SchoolImage[0]['School']['image_url'] == "" || !is_filecho($this->Html->url('/app/webroot/img/files/users/'.$SchoolImage[0]['School']['image_url'])) ) {
            $SchoolImage[0]['School']['image_url'] = "noImage.jpg";
        }
    ?>  
    <a href="<?php echo($this->Html->url('/schools/detail/'.$SchoolImage[0]['School']['id'])); ?>"><img alt="" border="0" src="<?php echo($this->Html->url('/thumbs/index/')); ?>?src=/app/webroot/img/files/schools/<?php echo $SchoolImage[0]['School']['image_url'];?>&w=150&h=150" /></a>
	</div>
...
echo $this->Html->image(('star-'.$SchoolImage[0]['School']['rating'].'.gif',array('alt'=>''));}?></div>
	<div class="textReviewdetails left"><strong>Total Review:</strong><?php echo $Firstreview ?></div>
	
	<?php /*?><?php if(isset($loggedInId)) {?>
	
	<div class="textReviewdetails left"><a href="<?php echo($this->Html->url('/reviews/uploadvideoreview'));?>/<?php echo $SchoolImage[0]['School']['id']; ?>"><?php echo $this->Html->image(('button.gif',array('alt'=>'')); ?></a></div>
	
	<?php } else { ?>
	
	<div class="textReviewdetails left"><a href="javascript:void(0);" class="loginBoxPopup"><?php echo $this->Html->image(('button.gif',array('alt'=>'')); ?></a></div>
	...
	<?php if(isset($loggedInId)) {?>
	    <?php if(!isset($MyReview)){ ?>
			...
			<div class="textReviewdetails noBorder"><a href="<?php echo($this->Html->url('/reviews/uploadreview_step'));?>/<?php echo $SchoolImage[0]['School']['id']; ?>"><?php echo $this->Html->image(('write-a-review.gif',array('alt'=>'')); ?></a></div>
			<?php } ?>
	<?php } else { ?>
		<?php if (isset($ApprovalPending)) {?> 
			<div class="textReviewdetails noBorder">Your review is being reviewed by a moderator</div>
		<?php } else { ?>
		<div class="textReviewdetails left"><a href="<?php echo($this->Html->url('/reviews/uploadreview_step'));?>/<?php echo $SchoolImage[0]['School']['id']; ?>"><?php echo $this->Html->image(('modify.gif',array('alt'=>'')); ?></a></div>
			<?php } ?>
	<?php }  ?>
	<?php } else { ?>
			<div class="textReviewdetails noBorder"><a href="javascript:void(0)" class="loginBoxPopup"><?php  echo $this->Html->image(('write-a-review.gif',array('alt'=>''));?></a></div>	
	<?php } ?>
		...
		<div class="info">
			<div class="inner">
				<div class="innerContent">
				 <span class="videoThumb"><a href="<?php echo($this->Html->url('/reviews/videoreview/'.$allvideoreview['Review']['id'].'')); ?>"><?php echo $VideoReviewCaps[$i]['image_thumb'][0]; ?></a></span>
				<p><?php echo $showIntro=substr($allvideoreview['Review']['description'],0,150); ?>...</p>
				<div class="clear"></div>
				<span class="reviewDate"><a href="<?php echo($this->Html->url('/reviews/videoreview/'.$allvideoreview['Review']['id'].'')); ?>" class="home_read"><div class="read_more left">Read more</div></a> &nbsp;|  Posted on <?php echo datecho("d M, Y ", strtotimecho($allvideoreview['Review']['created'])); ?> by <?php echo ucwords($allvideoreview['User']['firstname']); ?></span>
				</div>                                                                                                                
				<div class="clear"></div>
			</div>
		</div>
